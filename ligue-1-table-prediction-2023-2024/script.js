...
const teams = teamModel().getByLeague('ligue 1')
...
teams.forEach((team, i) => {
  ...
  liEl1.innerHTML = `
      <div class="flex items-center justify-center draggable js-drag" id="drag${i + 1}" data-name="${team.name}" data-index="${i}">
        <img class="h-6 w-6 md:h-10 md:w-10" src="../logos/france/${team.logo}" alt="logo ${team.name}">
      </div>
  `
      ...
      if (childrenEl) {
        ...
        if (relatedParent) {          
          ...
          childrenEl.innerHTML = `
            <img class="h-6 w-6 md:h-10 md:w-10" src="../logos/france/${currentTeam.logo}" alt="logo">
          `;
          ...
          event.relatedTarget.innerHTML = `
            <img class="w-4 h-4 md:w-8 md:h-8 mr-2" src="../logos/france/${team.logo}" alt="logo">
...
$saveBtn.addEventListener('click', (e) => {
  ...
  .then(function (dataUrl) {
    download(dataUrl, 'ligue_1_predictions_2023_2024.png')
